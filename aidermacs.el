...
(defun aidermacs--show-ediff-for-edited-files (edited-files)
  "Show differences for EDITED-FILES using the selected diff provider.
If `aidermacs-diff-provider' is set to `ediff', use ediff.
If set to `smerge', use smerge within the file buffer."
  (when edited-files
